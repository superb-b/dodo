...
#define Motor_id uint32_t
    ...
    // Previous CAN_Receive_Frame ，not actually used in SocketCAN implementation, but kept for reference and compatibility
    typedef struct
    {
        uint8_t FrameHeader;
...
#pragma pack()
    ...
    public:
        ...
        Motor(DM_Motor_Type Motor_Type, Motor_id Slave_id, Motor_id Master_id)
            ...
            this->limit_param = damiao_can::limit_param[Motor_Type];
        }

        Motor() : Master_id(0x01), Slave_id(0x11), Motor_Type(DM4310) {
            this->limit_param = damiao_can::limit_param[DM4310];
        }
 ...
 *
 *  SocketCAN + CanPort，directly on can0 
 */
    class Motor_Control
    {
    public:
        ...
        */
        ...
        void process_frame(uint32_t canId,
                        const uint8_t* data,
                        uint8_t len)
        {
            if(len < 6)
            ...
            // ===============================
            // 1️⃣ 电机状态反馈帧（位置/速度/力矩）
            // ===============================
            uint32_t motor_id = canId;
            ...
            static auto uint_to_float =
            [](uint16_t x, float xmin, float xmax, uint8_t bits)
            {
            ...
            auto m = it->second;
            Limit_param limit = m->get_limit_param();
            ...
            float q   = uint_to_float(q_uint,
                                    -limit.Q_MAX,
                                    limit.Q_MAX,
                                    16);

            float dq  = uint_to_float(dq_uint,
                                    -limit.DQ_MAX,
                                    limit.DQ_MAX,
                                    12);

            float tau = uint_to_float(tau_uint,
                                    -limit.TAU_MAX,
                                    limit.TAU_MAX,
                                    12);

            m->receive_data(q, dq, tau);
